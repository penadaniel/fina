...
const FormularioProductos = ({producto}) => {
    const {id, brand, price, photography, description, quantity } = producto;
            ...
            <div className="p-5 shadow-md bg-white">
                <div className="lg:flex">
                    ...
                    <div className="lg:w-7/12 xl:w-9/12 pl-5">
                        ...
                        <button onClick={ () => borrarProducto(producto.id)} type="submit" className="bg-red-800 hover:bg-gray-900 w-full mt-5 p-2 text-white uppercase font-bold">Borrar</button>
                        {/* <button type="submit" className="bg-gray-800 hover:bg-gray-900 w-full mt-5 p-2 text-white uppercase font-bold"></button> */}
                        <Link to={`/actualizar-producto/${producto.id}`}>Actualizar Producto</Link>
                    </div>
                </div>
            </div>
        </div>
